...
async function runTest() {
  ...
  try {
    ...
    const adminMe = await axios.get(`${API_URL}/auth/me`, { headers: { Authorization: `Bearer ${adminToken}` } });
    
    // Find Priya's ID by hitting the trainees endpoint
    const trainees = await axios.get(`${API_URL}/trainees`, { headers: { Authorization: `Bearer ${adminToken}` } });
    const traineeId = trainees.data[0].id;
    
    // 2. Initial Dashboard State
    console.log('1. Fetching Initial Analytics...');
    const initialDash = await axios.get(`${API_URL}/analytics/dashboard`, { headers: { Authorization: `Bearer ${adminToken}` } });
    ...
    const classifyReq = await axios.post(`${API_URL}/analytics/classify-reason`, {
    ...
    const riskReq = await axios.get(`${API_URL}/analytics/contactability-risk/${traineeId}`, { headers: { Authorization: `Bearer ${adminToken}` } });
    console.log(`   Risk Score: ${riskReq.data.risk_level}`);

    // Skill Gaps
    console.log('4. Fetching Skill Gaps...');
    const skillsReq = await axios.get(`${API_URL}/analytics/skill-gaps`, { headers: { Authorization: `Bearer ${adminToken}` } });
