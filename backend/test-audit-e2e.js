...
async function runAuditE2ETest() {
    ...
    try {
        ...
        const adminLogin = await axios_1.default.post(`${API_URL}/auth/login`, { username: 'admin', password: 'admin123' });
        const adminToken = adminLogin.data.token;
        assert(adminLogin.status === 200 && adminLogin.data.role === 'SYSTEM_ADMIN', 'Admin Login Successful with SYSTEM_ADMIN role');
        const priyaLogin = await axios_1.default.post(`${API_URL}/auth/login`, { username: 'priya_sharma', password: 'admin123' });
        ...
        const rameshLogin = await axios_1.default.post(`${API_URL}/auth/login`, { username: 'ramesh_kumar', password: 'admin123' });
        ...
        try {
            await axios_1.default.get(`${API_URL}/trainees/${rameshId}`, {
        ...
        try {
            await axios_1.default.post(`${API_URL}/outcomes/${rameshId}`, {
        ...
        const selfProfile = await axios_1.default.get(`${API_URL}/trainees/${priyaId}`, {
            headers: { Authorization: `Bearer ${priyaToken}` }
        });
        ...
        await axios_1.default.post(`${API_URL}/consent/${priyaId}`, {
        ...
        try {
            await axios_1.default.post(`${API_URL}/outcomes/${priyaId}`, {
                status: 'FORMAL_EMPLOYMENT',
                role: 'Test Role'
            }, {
                headers: { Authorization: `Bearer ${adminToken}` }
            ...
            assert(err.response?.status === 403 && err.response?.data?.error === 'CONSENT_REQUIRED_OR_REVOKED', 'Outcome intake rejected with HTTP 403 CONSENT_REQUIRED_OR_REVOKED when consent is revoked');
        }
        ...
        try {
            await axios_1.default.post(`${API_URL}/follow-ups/trigger`, {
                traineeId: priyaId,
            ...
            assert(err.response?.status === 403 && err.response?.data?.status === 'CANCELLED', 'Follow-up outreach rejected and state marked CANCELLED when consent is revoked');
        }
        ...
        const outcomeRes = await axios_1.default.post(`${API_URL}/outcomes/${priyaId}`, {
            status: 'FORMAL_EMPLOYMENT',
        ...
        const updatedProfile = await axios_1.default.get(`${API_URL}/trainees/${priyaId}`, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        ...
        const verifyReq = await axios_1.default.post(`${API_URL}/verification/request/${empRecordId}`, {}, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        ...
        const confirmRes = await axios_1.default.post(`${API_URL}/verification/confirm`, {
            verificationId,
            otp: demo_otp,
            status: 'CONFIRMED',
        ...
        const finalProfile = await axios_1.default.get(`${API_URL}/trainees/${priyaId}`, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        ...
        const dashRes = await axios_1.default.get(`${API_URL}/analytics/dashboard`, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        ...
        const pubRes = await axios_1.default.get(`${API_URL}/analytics/public-policy`);
        const smallCell = pubRes.data.districts.find((d) => d.district.includes('Jaisalmer'));
        ...
        const auditRes = await axios_1.default.get(`${API_URL}/audit`, {
            headers: { Authorization: `Bearer ${adminToken}` }
        });
        ...
        assert(actions.some((a) => a.startsWith('CONSENT_')), 'Audit trail logs CONSENT events');
