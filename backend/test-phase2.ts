...
async function runTest() {
  ...
  try {
    ...
    const traineeAuth = await axios.post(`${API_URL}/auth/login`, { username: 'priya_sharma', password: 'admin123' });
    const traineeToken = traineeAuth.data.token;
    
    // Get Trainee Profile
    const me = await axios.get(`${API_URL}/auth/me`, { headers: { Authorization: `Bearer ${traineeToken}` } });
    ...
    const outcomeResp = await axios.post(`${API_URL}/outcomes/${traineeId}`, {
    ...
    const profile = await axios.get(`${API_URL}/trainees/${traineeId}`, { headers: { Authorization: `Bearer ${traineeToken}` } });
    ...
    const adminAuth = await axios.post(`${API_URL}/auth/login`, { username: 'admin', password: 'admin123' });
    const adminToken = adminAuth.data.token;

    // 4. Admin Generates Verification Request (Simulating Employer Outreach)
    console.log('4. Generating Verification Request...');
    const verifyReq = await axios.post(`${API_URL}/verification/request/${employmentRecordId}`, {}, {
    ...
    await axios.post(`${API_URL}/verification/confirm`, {
      verificationId,
      otp,
      status: 'CONFIRMED',
    ...
    const finalProfile = await axios.get(`${API_URL}/trainees/${traineeId}`, { headers: { Authorization: `Bearer ${adminToken}` } });
