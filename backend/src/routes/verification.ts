...
router.post('/request/:employmentRecordId', authenticate, async (req: AuthRequest, res) => {
  try {
...
router.post('/confirm', async (req, res) => {
  try {
    ...
    if (status === 'CONFIRMED') {
      const v = await prisma.employerVerification.findUnique({ where: { id: verificationId } });
      if (v) {
        await prisma.wageRecord.updateMany({
          where: { employment_record_id: v.employment_record_id },
          data: { verification_status: 'VERIFIED' }
        });
        
        // Update OutcomeEvent provenance
        const emp = await prisma.employmentRecord.findUnique({ where: { id: v.employment_record_id }});
        if (emp) {
            await prisma.outcomeEvent.update({
