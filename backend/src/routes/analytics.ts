...
router.get('/dashboard', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const certifiedCount = await prisma.certificationRecord.count();
    const enrolledCount = await prisma.trainingEnrolment.count();
    ...
    const verifiedOutcomes = outcomes.filter(
      o => o.source === 'EMPLOYER_VERIFIED' || o.source === 'SYSTEM_DERIVED'
    );
    ...
    const totalBouncedEvents = await prisma.contactabilityEvent.count({ where: { event_type: 'BOUNCE' } });
...
router.get('/skill-gaps', authenticate, async (req: AuthRequest, res) => {
  try {
    const feedback = await prisma.employerSkillFeedback.findMany({
      include: { skill: true }
    });
...
router.get('/wage-progression', authenticate, async (req: AuthRequest, res) => {
  try {
...
router.post('/classify-reason', authenticate, async (req: AuthRequest, res) => {
  try {
...
router.get('/contactability-risk/:traineeId', authenticate, async (req: AuthRequest, res) => {
  try {
    const { traineeId } = req.params;
    
    const events = await prisma.contactabilityEvent.findMany({
      where: { trainee_id: traineeId }
    });

    const alternateContacts = await prisma.alternateContact.findMany({
      where: { trainee_id: traineeId }
    });
...
router.get('/public-policy', async (req, res) => {
  try {
    const totalCertified = await prisma.certificationRecord.count();
    const outcomes = await prisma.outcomeEvent.findMany();
...
router.get('/export', authenticate, async (req: AuthRequest, res) => {
  try {
    const certified = await prisma.certificationRecord.count();
    const outcomes = await prisma.outcomeEvent.count();
