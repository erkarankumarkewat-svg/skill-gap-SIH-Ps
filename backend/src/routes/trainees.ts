...
router.get('/', authenticate, requireRole(['SYSTEM_ADMIN', 'DISTRICT_ADMIN', 'PROVIDER']), async (req: AuthRequest, res) => {
  try {
    const where: any = {};
...
router.post('/', authenticate, requireRole(['SYSTEM_ADMIN', 'PROVIDER']), async (req: AuthRequest, res) => {
  try {
