  ...
  try {
    ...
    const enriched = providers.map((p) => {
      ...
      for (const t of trainees) {
        if (!t) continue;
        const verifiedOutcomes = t.outcome_events.filter(
      ...
      const compositeScore = Math.round(
        verifiedPlacementRate * DEFAULT_WEIGHTS.placementWeight +
...
router.get('/:id/scorecard', authenticate, async (req, res) => {
  try {
    ...
    const verifiedCount = allEnrolments.filter(e =>
      e.trainee?.outcome_events.some(o => o.source === 'EMPLOYER_VERIFIED' || o.source === 'SYSTEM_DERIVED')
    ).length;
    ...
    const compositeScore = Number((
      verifiedPlacement * DEFAULT_WEIGHTS.placementWeight +
      retention6m * DEFAULT_WEIGHTS.retention6mWeight +
      retention12m * DEFAULT_WEIGHTS.retention12mWeight +
      wageProgression * DEFAULT_WEIGHTS.wageProgressionWeight +
      responseQuality * DEFAULT_WEIGHTS.responseQualityWeight
